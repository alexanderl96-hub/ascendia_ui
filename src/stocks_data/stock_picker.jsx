...
// import { getStocks, getStockPrices } from "../lib/api"; 
...
// const stocksPrices = await getStockPrices()
// const PAGE_SIZE = 15; 
...
import { useNavigate } from "react-router-dom";
import { getStocks, getStockPrices } from "../lib/api";
import { getSymbolIcon } from "../lib/symbolIcons";
import "./stock-picker.css";
...
export default function StockPicker({ minPick = 3, onContinue }) {
  ...
  const [pricesBySymbol, setPricesBySymbol] = useState({}); // { AAPL: {...}, TSLA: {...} }
    ...
    (async () => {
      try {
        ...
        const [stocksRes, pricesRes] = await Promise.all([
          getStocks({ signal: ac.signal }),
          getStockPrices({}, { signal: ac.signal }), // ✅ whole data from /api/stock-price
        ]);

        setStocks(Array.isArray(stocksRes) ? stocksRes : []);

        const map = {};
        (Array.isArray(pricesRes) ? pricesRes : []).forEach((row) => {
  ...
  // ✅ Merge stock list + price table data by symbol
  // Assumes StockPriceRowDTO returns fields like:
  // { symbol, currentPrice, percentChange, ... }
  const mergedStocks = useMemo(() => {
    return stocks.map((s) => {
      ...
      const livePrice =
        p?.currentPrice ?? s.lastStrategyPrice ?? null;

      const changePct =
        p?.percentChange ?? null; // already a number (e.g. -1.23 means -1.23%)
  ...
  if (loading) return <div className="sp__page"><div className="sp__wrap">Loading...</div></div>;
  if (error) return <div className="sp__page"><div className="sp__wrap" style={{ color: "red" }}>{error}</div></div>;
        ...
        <section className="sp__section">
          ...
          <div className="sp__grid">
            {pageItems.map((s) => {
              ...
              const pct = typeof s.changePct === "number" ? s.changePct : 0;
              const up = pct >= 0;

              const priceNumber =
                typeof s.livePrice === "number"
                  ? s.livePrice
                  : s.livePrice != null
                    ? Number(s.livePrice)
                    : null;
                ...
                >
                  <div className="sp__cardTop">
                    <div className="sp__identity">
                      <div className="sp__symbol">
                        <img
                          className="sp__symIcon"
                          src={getSymbolIcon(s.symbol)}
                          alt=""
                          aria-hidden="true"
                        />
                        {s.symbol}
                    ...
                    <div className={`sp__check ${isSelected ? "on" : ""}`}>✓</div>
                  </div>
